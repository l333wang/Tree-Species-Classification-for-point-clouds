...
def knn_point(radius, nsample, xyz, new_xyz):
    """
    Input:
    ...
    # sqrdists = sqrdists.permute(0,2,1) # b s n
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False):
    ...
    idx = knn_point(radius, nsample, xyz, new_xyz)
...
class PointNetSetAbstractionMsg_xyz(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        super(PointNetSetAbstractionMsg_xyz, self).__init__()
        self.npoint = npoint
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            group_idx = knn_point(radius, K, xyz, new_xyz)
            grouped_xyz = index_points(xyz, group_idx)
            # print(new_xyz.shape, grouped_xyz.shape)
        ...
        return new_xyz, new_points_concat, grouped_points_list, fps_index

class PointNetSetAbstractionMsg_others(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        super(PointNetSetAbstractionMsg_others, self).__init__()
        self.npoint = npoint
        self.radius_list = radius_list
        self.nsample_list = nsample_list
        self.conv_blocks = nn.ModuleList()
    ...
    def forward(self, xyz, points, s_index):
        """
        Input:
            xyz: input points position data, [B, C, N]
            points: input points data, [B, D, N]
        ...
        new_xyz = index_points(xyz, s_index)
        new_points_ori = index_points(points, s_index) # B S D
