...
from models.GTN.pointnet2_utils import InputEmbedding, PointNetSetAbstractionMsg, PointNetSetAbstraction, Global_Transformer
import torch
import numpy as np
...
class get_model(nn.Module):
    def __init__(self, num_class, geometric_channel, feature_channel, sampling_rate, npoints):
        ...
        in_channel = len(self.channel_idx)
        self.embedding = InputEmbedding([0.2], [16], in_channel, [[32, 32, 64]])
        # sampling_points[0], [0.1, 0.2, 0.4], [16, 32, 128], 64,
        #   256,64                                           [[32, 32, 64], [64, 64, 128], [64, 96, 128]]
        self.sa1 = PointNetSetAbstractionMsg(sampling_points[0], [0.1, 0.2, 0.4], [16, 32, 128], 64,
                                             [[32, 32, 64], [64, 64, 128], [64, 96, 128]])
        self.global_sa1 = Global_Transformer(avepooling=False, batchnorm=True, attn_drop_value=0, feed_drop_value=0,
                                             npoint=sampling_points[0], in_channel=320, out_channels=320, layers=1, num_heads=8,
                                             head_dim=40)
        self.sa2 = PointNetSetAbstractionMsg(sampling_points[1], [0.2, 0.4, 0.8], [32, 64, 128], 320,
                                             [[64, 64, 128], [128, 128, 256], [128, 128, 256]])
        self.global_sa2 = Global_Transformer(avepooling=False, batchnorm=True, attn_drop_value=0, feed_drop_value=0,
                                             npoint=sampling_points[1], in_channel=640, out_channels=640, layers=1, num_heads=16,
                                             head_dim=40)
    ...
    def forward(self, xyz):
        ...
        if (0 not in self.channel_idx) or (1 not in self.channel_idx) or (2 not in self.channel_idx):
            raise Exception("No enough coordinate features!!! Please try again")

        norm = xyz[:, self.channel_idx, :]
        xyz = xyz[:, :3, :]
