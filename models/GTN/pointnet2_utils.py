...
class WindowAttention(nn.Module):
    def __init__(self, input_dim, output_dim, heads, head_dim, N, attn_drop_value):
        ...
        self.linear_input = nn.Sequential(nn.Linear(input_dim, output_dim), nn.BatchNorm2d(output_dim),
        ...
        self.value_conv = nn.Sequential(
            nn.Conv1d(in_channels=input_dim, out_channels=input_dim, kernel_size=1, bias=False),
            self.bn3,
            nn.ReLU())
    ...
    def forward(self, x, p_r):
        ...
        for i, layer in enumerate(self.linear_input): x = layer(x.permute(0, 3, 1, 2)).permute(0, 2, 3,
                                                                                               1) if i == 1 else layer(
            x)
        ...
        for i, layer in enumerate(self.linear_p): p_r = layer(p_r.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4,
                                                                                                  1) if i == 1 else layer(
            p_r)
        # b,n,k,k,1
...
class SwinBlock(nn.Module):
    def __init__(self, batchnorm, input_dim, output_dim, heads, head_dim, mlp_dim, patch_size, attn_drop_value,
                 feed_drop_value):
        super().__init__()
...
class Global_Transformer(nn.Module):
    ...
    def __init__(self, avepooling, batchnorm, attn_drop_value, feed_drop_value, npoint, in_channel, out_channels,
                 layers, num_heads, head_dim):
        super(Global_Transformer, self).__init__()
        self.npoint = npoint
        self.avepooling = avepooling

        self.layer = SwinBlock(batchnorm, input_dim=in_channel, output_dim=out_channels, heads=num_heads,
                               head_dim=head_dim, mlp_dim=in_channel * 4,
    ...
    def forward(self, xyz, points, grouped_points_list):
        ...
        output_points = shortcut.permute(0, 2, 1) + self.act(
            self.after_norm(self.trans_conv(output_points.permute(0, 2, 1))))  # b d n
        #####################################################################
